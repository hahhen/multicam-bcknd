...
const namespace_langs = require('instaview/lib/namespace_langs');
    ...
    InstaView.conf =
    {
        ...
        paths: {
            base_href: '/',
            articles: 'http://camera-wiki.org/wiki/',
            math: '/math/',
            images: '',
    ...
    InstaView.convert = function (wiki) {
        ...
        function parse_inline_images(str) {
            const regex = /{{.*?\|\s*image[ _]source\s*=\s*(.*?)\s*\|\s*image\s*=\s*(.*?)\|.*?}}/;
            const match = str.match(regex);

            if (match) {
                const imageSource = match[1];
                const imageUrl = match[2];
                return `<br><a rel="nofollow" class="external text" href="${imageSource}"><img src="${imageUrl}" alt="${imageUrl.split('/').pop()}"></a><br>`;
            }
